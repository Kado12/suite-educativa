...
import { CreateSectionDto, UpdateSectionFullDto, UpdateSectionDto } from './dto/section.dto';
...
export class AcademicService {
  ...
  async deleteSede(id: string) {
    ...
    if (s.classrooms.length > 0)
      throw new ConflictException('La sede tiene salones');
    return this.prisma.sede.delete({ where: { id } });
  }

  // ===== TURNOS =====
  async createTurno(d: CreateTurnoDto) {
    if (await this.prisma.turno.findUnique({ where: { name: d.name } }))
  ...
  async updateTurno(id: string, d: UpdateTurnoDto) {
  ...
  async deleteTurno(id: string) {
    ...
    if (t.sections.length > 0) 
      throw new ConflictException('El turno tiene secciones');
  ...
  async createClassroom(name: string, sedeId: string) {
    if (await this.prisma.classroom.findFirst({ where: { name, sedeId } }))
      throw new ConflictException('Salón ya existe en la sede');
    return this.prisma.classroom.create({ data: { name, sedeId } });
  }

  async updateClassroom(id: string, d: UpdateClassroomDto) {
  ...
  async deleteClassroom(id: string) {
    ...
    if (c.sections.length > 0) 
      throw new ConflictException('El salón tiene secciones');
    return this.prisma.classroom.delete({ where: { id } });
  }

  // ===== SECCIONES =====
  async createSection(d: CreateSectionDto) {
    ...
    if (!c || !t) 
      throw new NotFoundException('Salón o turno no encontrado');
  ...
  async updateSection(id: string, d: UpdateSectionDto) {
  ...
  //! DOS METODOS IGUALES
  async updateSectionFull(id: string, d: UpdateSectionFullDto) {
    const s = await this.prisma.section.findUnique({ where: { id } });
    if (!s)
      throw new NotFoundException('Sección no encontrada');
    return this.prisma.section.update({ where: { id }, data: d });
  ...
  async deleteSection(id: string) {
    ...
    if (count > 0)
      throw new ConflictException('La sección tiene matrículas');
  ...
  async createArea(name: string) {
    if (await this.prisma.area.findUnique({ where: { name } }))
      throw new ConflictException('Área ya existe');
    return this.prisma.area.create({ data: { name } });
  }

  async listAreas() {
    return this.prisma.area.findMany({ include: { courses: true }, orderBy: { name: 'asc' } });
  }
  ...
  async deleteArea(id: string) {
    ...
    if (!a)
      throw new NotFoundException('Área no encontrada');
    if (a.courses.length > 0)
      throw new ConflictException('El área tiene cursos');
  ...
  async updateCourse(id: string, d: UpdateCourseDto) {
  ...
  async deleteCourse(id: string) {
    ...
    if (uses > 0)
      throw new ConflictException('El curso está asignado a docentes');
    return this.prisma.course.delete({ where: { id } });
  }

  // ===== PERÍODOS / BLOQUES =====
  async createPeriod(d: CreatePeriodDto) {
    if (await this.prisma.period.findUnique({ where: { name: d.name } }))
      throw new ConflictException('Período ya existe');
    const date = new Date(`${d.startDate}T00:00:00Z`);
    if (date.getUTCDay() !== 1)
      throw new BadRequestException('La fecha debe ser LUNES');
    return this.prisma.period.create({ 
      data: { 
        name: d.name, startDate: date, weeks: d.weeks || 12,
        isActive: true
      }
    });
  }
  ...
  async updatePeriod(id: string, d: UpdatePeriodDto) {
    ...
    if (d.weeks) {
      if (d.weeks < 1)
        throw new BadRequestException('Debe tener al menos 1 semana');
  ...
  async deletePeriod(id: string) {
    ...
    if (b > 0)
      throw new ConflictException('El período tiene bloques');
  ...
  async updateBlock(id: string, d: UpdateBlockDto) {
    ...
    if (d.name && d.name !== b.name) {
      ...
      if (exists)
        throw new ConflictException('Ya existe un bloque con ese nombre en el período');
      updateData.name = d.name;
    }
  ...
  async deleteBlock(id: string) {
    ...
    if (s > 0)
      throw new ConflictException('El bloque tiene sesiones');
  ...
  async addCourseToBlock(blockId: string, courseId: string) {
    return this.prisma.blockCourse.upsert({
      where: {
        blockId_courseId: { blockId, courseId }
      },
      update: {},
  ...
  // ===== PLANES DE PAGO =====
  async createPaymentPlan(d: CreatePaymentPlanDto): Promise<any> {
    if (await this.prisma.paymentPlan.findUnique({ where: { name: d.name } })) {
      throw new ConflictException('Plan ya existe');
    }
    if (d.installments < 1)
      throw new BadRequestException('Debe tener al menos 1 cuota');
    return this.prisma.paymentPlan.create({
      data: { name: d.name, installments: d.installments, amount: d.amount }
    });
  }
  ...
  async updatePaymentPlan(id: string, d: UpdatePaymentPlanDto): Promise<any> {
  ...
  async deletePaymentPlan(id: string): Promise<any> {
    ...
    if (uses > 0)
      throw new ConflictException('El plan tiene pagos asociados. Desactívalo en su lugar.');
